...
class LowQualityButGroundedAdapter(LocalMockRAGAdapter):
    """Demo adapter that preserves citations but produces low-value answers."""

    name = "human_review_demo_low_generation_quality"

    async def ask(self, request: RagRequest) -> RagResponse:
        response = await super().ask(request)
        response.answer = (
            "الإجابة غير كافية لاتخاذ قرار نهائي. تحتاج هذه الحالة إلى مراجعة بشرية "
            "ومقارنة إضافية مع السياسة المعتمدة قبل الاعتماد."
        )
        response.raw["human_review_demo"] = "low_generation_quality"
        return response


def main() -> None:
    parser = argparse.ArgumentParser(description="Run an intentional HUMAN_REVIEW_REQUIRED demo.")
    ...
    parser.add_argument("--out", default="reports/human_review_evaluation_report.json")
    parser.add_argument("--markdown-out", default="reports/human_review_release_report.md")
    parser.add_argument("--sqlite-db", default="reports/evaluation_history.sqlite")
    args = parser.parse_args()

    report = evaluate_dataset(LowQualityButGroundedAdapter(), args.dataset, args.thresholds)
    ...
    if report.decision != "HUMAN_REVIEW_REQUIRED":
        raise SystemExit(f"Expected HUMAN_REVIEW_REQUIRED, got {report.decision}")
